...
fun main() {
    ...
    fun neighbours(r: Reference, map: Matrix<Char>) =
    ...
    fun nodesDiamond(maxSteps: Int): Set<Pair<Reference, Int>> {
        ...
        while (steps <= maxSteps && currentNodes.isNotEmpty()) {
            ...
            currentNodes = currentNodes.flatMap { it.plusAll().values }
    ...
    fun part2(input: Matrix<Char>, maxSteps: Int, target: Int): Int {
            val start = input.findChar('S')
            val end = input.findChar('E')

            val (costs, path) = graphSSPWithBT(
                start, input,
