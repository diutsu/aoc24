...
fun main() {
    ...
    fun walkSameFrom(i: Int, j: Int, test: Char, input: List<List<Char>>, visited: MutableMatrix<Boolean>) : Pair<Int, Int> {
    ...
    fun walkSameFromWithCorners(i: Int, j: Int, test: Char, input: List<List<Char>>, visited: MutableMatrix<Boolean>) : Pair<Int, Int> {
        fun matches(a: Reference) = a in input && input[a] == test

        fun getNumberOfCorners(
            point: Reference,
        ): Int {
...
//            println("Found ${numberOfCorners-a} corners")
        ...
        fun countCorners(reference: Reference, entry: EnumMap<CardinalDirections, Reference>): Int {
             return CardinalDirections.entries.count { dir ->
